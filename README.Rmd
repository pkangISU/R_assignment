...
#make maize_t and snp_3 to data frame type,so that we can use some function
```{r}
...
```
#sort each chromosome file from maize by decreasing position with missing value as -
```{r}
...
```{r}
library(ggplot2)
summarise(maize_all,group_by(Chromosome),count = n())
maize_g <- maize_all %>% group_by(Chromosome) %>% summarise(total_snp=n())
View(maize_g)
ggplot(data=maize_g,mapping=aes(x=Chromosome, y=total_snp)) + geom_bar(width = 1,stat = "identity") + ggtitle("maize snp distribution across chromosomes")
```
